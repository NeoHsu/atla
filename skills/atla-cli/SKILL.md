...
compatibility: Requires atla CLI 0.8.0 exactly
---
...
This skill release targets exactly `atla 0.8.0`, as recorded in `compatibility.json`. Before auth,
credentials, network access, or any Atlassian operation, run:

```bash
atla doctor --skill-version 0.8.0 --output json
```
...
this skill requires `0.8.0`, and offer
`cargo install --locked --git https://github.com/NeoHsu/atla --tag v0.8.0 atla` with the same
