...
mod tests {
    ...
    fn command_index_skips_dry_run_flag() {
        assert_eq!(command_index(&args(&["atla", "--dry-run", "jira"])), Some(2));
    }
    ...
    fn command_index_unknown_flag_treated_as_command() {
        assert_eq!(
            command_index(&args(&["atla", "--unknown-flag"])),
            Some(1)
        );
    }
    ...
    fn expands_alias_no_trailing_args() {
        ...
        let expanded =
            expand_args_with_config(args(&["atla", "ls"]), &config).expect("expand");
    ...
    fn alias_expansion_limit_error() {
        ...
        config
            .aliases
            .insert("a9".to_owned(), "a1".to_owned());
    ...
    fn empty_alias_expansion_errors() {
        ...
        config
            .aliases
            .insert("empty".to_owned(), "".to_owned());
    ...
    fn command_index_multiple_flags_then_command() {
        assert_eq!(
            command_index(&args(&["atla", "--verbose", "--dry-run", "--no-input", "jira"])),
            Some(4)
        );
    }
    ...
    fn command_index_multiple_value_flags_then_command() {
        assert_eq!(
            command_index(&args(&[
                "atla", "--output", "json", "--profile", "work", "jira"
            ])),
            Some(5)
        );
    }
